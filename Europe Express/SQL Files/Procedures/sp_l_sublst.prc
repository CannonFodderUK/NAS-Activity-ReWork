SET DEFINE OFF;
/* Formatted on 16/01/2013 11:25:39 (QP5 v5.227.12220.39724) */
CREATE OR REPLACE PROCEDURE sp_l_sublst
AS
   -- DECLARE /*****  D E C L A R E  S E C T I O N  *****/
   --===========================================================
   -- SP_L_SUBLST - which takes the contents of L_MRKSUBSCRIPTIONLISTS
   -- and populates the D_SUBSCRIPTION_LISTS table
   --
   --===========================================================
   -- Change History
   --===========================================================
   -- Date          Ver Who Comment
   -- 07-Jan-13     1.0 tthmlx  Original Version
   --
   --
   -- EXCEPTION DEFINITIONS
   --
   primary_key_error      EXCEPTION;
   PRAGMA EXCEPTION_INIT (primary_key_error, -00001);
   foreign_key_error      EXCEPTION;
   PRAGMA EXCEPTION_INIT (foreign_key_error, -02291);
   --
   --
   v_commit_at   CONSTANT PLS_INTEGER := 1000;
   --
   -- VARIABLES HOLDING DATA FROM CURSOR
   --
   v_list_id              D_SUBSCRIPTION_LISTS.LIST_ID%TYPE;
   v_list_type            D_SUBSCRIPTION_LISTS.LIST_TYPE%TYPE;
   v_description          D_SUBSCRIPTION_LISTS.DESCRIPTION%TYPE;
   v_display_seq          D_SUBSCRIPTION_LISTS.DISPLAY_SEQ%TYPE;
   v_display_text         D_SUBSCRIPTION_LISTS.DISPLAY_TEXT%TYPE;
   v_brand_id             D_SUBSCRIPTION_LISTS.BRAND_ID%TYPE;
   v_affiliation_id       D_SUBSCRIPTION_LISTS.AFFILIATION_ID%TYPE;
   v_show_non_signup      D_SUBSCRIPTION_LISTS.SHOW_NON_SIGNUP%TYPE;
   v_selected_on_signup   D_SUBSCRIPTION_LISTS.SELECTED_ON_SIGNUP%TYPE;
   v_whostamp             D_SUBSCRIPTION_LISTS.WHOSTAMP%TYPE;
   v_datestamp            D_SUBSCRIPTION_LISTS.DATESTAMP%TYPE;
   --
   -- WORK VARIABLES
   --
   v_null                 CHAR (1);
   v_count                CHAR (1);
   v_insert_ok            CHAR (1);
   v_update_ok            CHAR (1);
   v_fk_error             CHAR (1);
   v_foreign_key_error    CHAR (1);
   v_primary_key_error    CHAR (1);
   --
   v_code                 NUMBER (5);
   v_error_message        VARCHAR2 (512);

   --
   CURSOR C001
   IS
      SELECT LISTID LIST_ID,
             LISTTYPE LIST_TYPE,
             DESCRIPTION,
             DISPLAYSEQ DISPLAY_SEQ,
             DISPLAYTEXT DISPLAY_TEXT,
             BRANDID BRAND_ID,
             AFFILIATIONID AFFILIATION_ID,
             SHOWNONSIGNUP SHOW_NON_SIGNUP,
             SELECTEDONSIGNUP SELECTED_ON_SIGNUP,
             WHOSTAMP,
             DATESTAMP
        FROM L_MRKSUBSCRIPTIONLISTS;

--
BEGIN
   /**** < OPEN Cursor Block > ****/
   --
   v_primary_key_error := 'N';
   v_foreign_key_error := 'N';

   --
   OPEN C001;

   --
   BEGIN
      /**** < FETCH Cursor Block >  ****/
      --
      LOOP
         /**** < FETCH LOOP      >  ****/
         --
         v_insert_ok := 'N';
         v_update_ok := 'N';
         v_fk_error := 'N';

         --
         FETCH C001
            INTO v_list_id,
                 v_list_type,
                 v_description,
                 v_display_seq,
                 v_display_text,
                 v_brand_id,
                 v_affiliation_id,
                 v_show_non_signup,
                 v_selected_on_signup,
                 v_whostamp,
                 v_datestamp;

         --
         EXIT WHEN C001%NOTFOUND;

         --
         BEGIN
            /**** < MAIN  Cursor Block >  ****/
            --
            WHILE     --
                      v_insert_ok = 'N'
                  AND v_update_ok = 'N'
                  AND v_fk_error = 'N'
            --
            LOOP
               /**** < RETRY LOOP      >  ****/
               --
               v_primary_key_error := 'N';
               v_foreign_key_error := 'N';

               --
               BEGIN
                  /**** < INSERT Cursor Block > ****/
                  --
                  INSERT INTO d_subscription_lists (list_id,
                                                    list_type,
                                                    description,
                                                    display_seq,
                                                    display_text,
                                                    brand_id,
                                                    affiliation_id,
                                                    show_non_signup,
                                                    selected_on_signup,
                                                    whostamp,
                                                    datestamp)
                       VALUES (v_list_id,
                               v_list_type,
                               v_description,
                               v_display_seq,
                               v_display_text,
                               v_brand_id,
                               v_affiliation_id,
                               v_show_non_signup,
                               v_selected_on_signup,
                               v_whostamp,
                               v_datestamp);

                  --
                  v_insert_ok := 'Y';
               --
               EXCEPTION
                  --
                  -- A PRIMARY KEY or FOREIGN KEY error
                  --
                  WHEN primary_key_error
                  THEN
                     v_primary_key_error := 'Y';
                  WHEN foreign_key_error
                  THEN
                     v_foreign_key_error := 'Y';
               END;                        /**** < INSERT Cursor Block > ****/

               --
               IF v_primary_key_error = 'Y'
               THEN
                  --
                  BEGIN
                     /**** < UPDATE Cursor Block > ****/
                     --
                     UPDATE D_SUBSCRIPTION_LISTS
                        SET LIST_TYPE = V_LIST_TYPE,
                            DESCRIPTION = V_DESCRIPTION,
                            DISPLAY_SEQ = V_DISPLAY_SEQ,
                            DISPLAY_TEXT = V_DISPLAY_TEXT,
                            BRAND_ID = V_BRAND_ID,
                            AFFILIATION_ID = V_AFFILIATION_ID,
                            SHOW_NON_SIGNUP = V_SHOW_NON_SIGNUP,
                            SELECTED_ON_SIGNUP = V_SELECTED_ON_SIGNUP,
                            WHOSTAMP = V_WHOSTAMP,
                            DATESTAMP = V_DATESTAMP
                      WHERE LIST_ID = V_LIST_ID;

                     --
                     v_update_ok := 'Y';
                  --
                  EXCEPTION
                     --
                     -- A FOREIGN KEY error
                     --
                     WHEN foreign_key_error
                     THEN
                        v_foreign_key_error := 'Y';
                  --
                  END;                     /**** < UPDATE Cursor Block > ****/
               --
               END IF;

               -- FOREIGN KEY problem
               IF v_foreign_key_error = 'Y'
               THEN
                  -- write error record
                  INSERT INTO R_sublst_rej_rec (list_id,
                                                list_type,
                                                description,
                                                display_seq,
                                                display_text,
                                                brand_id,
                                                affiliation_id,
                                                show_non_signup,
                                                selected_on_signup,
                                                whostamp,
                                                datestamp)
                       VALUES (v_list_id,
                               v_list_type,
                               v_description,
                               v_display_seq,
                               v_display_text,
                               v_brand_id,
                               v_affiliation_id,
                               v_show_non_signup,
                               v_selected_on_signup,
                               v_whostamp,
                               v_datestamp);

                  --
                  -- Now check which FOREIGN KEY is the problem
                  --
                  v_fk_error := 'Y';
               --
               END IF;
            --
            END LOOP;                      /**** < END RETRY LOOP     >  ****/
         --
         EXCEPTION
            --
            -- An error has occured that is not a PRIMARY or FOREIGN KEY error
            --
            WHEN OTHERS
            THEN
               --
               v_code := SQLCODE;
               v_error_message := SQLERRM;

               --
               INSERT INTO integration_errors (load_table_abbrev,
                                               target_table_name,
                                               error_date,
                                               ERROR_CODE,
                                               error_desc)
                    VALUES ('L_SUBLST',
                            'D_SUBSCRIPTION_LISTS',
                            SYSDATE,
                            SUBSTR (v_code, 1, 50),
                            SUBSTR (v_error_message, 1, 250));

               --
               -- Write error record
               INSERT INTO R_sublst_rej_rec (list_id,
                                             list_type,
                                             description,
                                             display_seq,
                                             display_text,
                                             brand_id,
                                             affiliation_id,
                                             show_non_signup,
                                             selected_on_signup,
                                             whostamp,
                                             datestamp)
                    VALUES (v_list_id,
                            v_list_type,
                            v_description,
                            v_display_seq,
                            v_display_text,
                            v_brand_id,
                            v_affiliation_id,
                            v_show_non_signup,
                            v_selected_on_signup,
                            v_whostamp,
                            v_datestamp);
         --
         END;                               /**** < MAIN Cursor Block >  ****/

         --
         -- Commit at intervals
         --
         IF C001%ROWCOUNT MOD v_commit_at = 0
         THEN
            --
            COMMIT;
         --
         END IF;
      --
      END LOOP;                            /**** < END FETCH LOOP     >  ****/
   --
   EXCEPTION
      --
      -- A serious error has occured
      --
      WHEN OTHERS
      THEN
         --
         v_code := SQLCODE;
         v_error_message := SQLERRM;

         --
         INSERT INTO integration_errors (load_table_abbrev,
                                         target_table_name,
                                         error_date,
                                         ERROR_CODE,
                                         error_desc)
              VALUES ('L_SUBLST',
                      'D_SUBSCRIPTION_LISTS',
                      SYSDATE,
                      SUBSTR (v_code, 1, 50),
                      SUBSTR (v_error_message, 1, 250));
   --
   --
   END;                                     /**** < FETCH Cursor Block > ****/

   --
   CLOSE C001;

   --
   -- Final commit
   COMMIT;
--
EXCEPTION
   --
   -- A serious error has occured
   --
   WHEN OTHERS
   THEN
      --
      v_code := SQLCODE;
      v_error_message := SQLERRM;

      --
      INSERT INTO integration_errors (load_table_abbrev,
                                      target_table_name,
                                      error_date,
                                      ERROR_CODE,
                                      error_desc)
           VALUES ('L_SUBLST',
                   'D_SUBSCRIPTION_LISTS',
                   SYSDATE,
                   SUBSTR (v_code, 1, 50),
                   SUBSTR (v_error_message, 1, 250));

      --
      COMMIT;
--
END;                                        /**** < OPEN Cursor Block  > ****/
/
SHOW ERRORS;
